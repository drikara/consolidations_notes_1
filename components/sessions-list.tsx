...
export function SessionsList({ sessions }: SessionsListProps) {
  ...
  const handleDelete = async (sessionId: string) => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Erreur lors de la suppression de la session')
    } finally {
      setLoading(null)
    }
  }
              ...
              <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white px-4 py-2 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 font-semibold text-sm text-center disabled:opacity-50"
                  >
                    {loading === session.id ? (
                      <>
