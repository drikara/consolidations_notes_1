    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border-2 border-orange-100 shadow-lg overflow-hidden">
        ...
        <div className="p-6">
          ...
          {activeTab === 'jury' && (
            <div>
              <h3 className="text-xl font-bold text-orange-800 mb-6">Membres du Jury Présents</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {session.juryPresences.map((presence, index) => (
                  <div
                    key={index}
                    className={`rounded-2xl p-6 border-2 shadow-sm transition-all duration-200 ${
                      presence.wasPresent 
                        ? 'bg-gradient-to-br from-emerald-50 to-white border-emerald-200 hover:shadow-lg' 
                        : 'bg-gradient-to-br from-red-50 to-white border-red-200 hover:shadow-lg'
                    }`}
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <div className="font-bold text-gray-900">{presence.juryMember.fullName}</div>
                        <div className="text-sm text-orange-600 mt-1">
                          {presence.juryMember.roleType}
                          {presence.juryMember.specialite && ` - ${presence.juryMember.specialite}`}
                        </div>
                      </div>
                      <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border-2 ${
                        presence.wasPresent 
                    ...
                    {!presence.wasPresent && presence.absenceReason && (
                      <div className="mt-4 p-3 bg-white rounded-xl border border-orange-200">
                        <div className="text-sm text-gray-700">
                          <strong className="text-orange-700">Raison :</strong> {presence.absenceReason}
                        </div>
                      </div>
                    )}
                  </div>
